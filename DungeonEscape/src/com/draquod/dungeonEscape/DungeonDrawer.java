...
public class DungeonDrawer {
	...
	public DungeonDrawer() {
		...
		//assets.load("data/corridorTile.g3db", Model.class);
		...
		assets.load("data/topTile.obj", Model.class);
		loading = true;
	}
	...
	public void doneLoading() {
		...
		//Model corridorTile = assets.get("data/corridorTile.g3db", Model.class);
		...
		Model topTile = assets.get("data/topTile.obj", Model.class);
			...
			for (int j = 1; j < dg.n_rows - 1; j++) {
				...
				if (dg.cells[i][j] != 0 && dg.cells[i][j] != dg.PERIMETER) {
					...
					if (dg.cells[i + 1][j] != 0
						...
						instance.transform.translate(i * dg.cell_size * scale,
								0, -j * dg.cell_size * scale);
						instance.transform.rotate(0, 1, 0, 180);
						instances.add(instance);
					} else if ((dg.cells[i + 1][j] == dg.PERIMETER || dg.cells[i + 1][j] == 0)
							&& dg.cells[i - 1][j] != 0
							&& dg.cells[i][j + 1] != 0
							&& (dg.cells[i][j - 1] == dg.PERIMETER || dg.cells[i][j - 1] == 0)) {
						instance = new ModelInstance(turnTile);
						...
						instance.transform.translate(i * dg.cell_size * scale,
								0, -j * dg.cell_size * scale);
						instance.transform.rotate(0, 1, 0, 0);
						instances.add(instance);
					} else if (dg.cells[i + 1][j] != 0
							&& (dg.cells[i - 1][j] == dg.PERIMETER || dg.cells[i - 1][j] == 0)
							&& (dg.cells[i][j + 1] == dg.PERIMETER || dg.cells[i][j + 1] == 0)
							&& dg.cells[i][j - 1] != 0) {
						instance = new ModelInstance(turnTile);
					...
					if (dg.cells[i + 1][j] != 0
						...
						instance.transform.rotate(0, 1, 0, 90);
						instances.add(instance);
					} else if ((dg.cells[i + 1][j] == 0 || dg.cells[i + 1][j] == dg.PERIMETER)
					...
					if (dg.cells[i][j] == dg.STAIR_DN) {
						instance = new ModelInstance(normalTile);
						instance.transform.translate(i * dg.cell_size * scale,
								0.5f, -j * dg.cell_size * scale);
						instance.transform.scale(0.8f, 0.8f, 0.8f);
						instance.materials.get(0).set(
								ColorAttribute.createDiffuse(0.4f, 1.0f, 0.4f,
										0.9f));
						instances.add(instance);
					}
					...
					if (dg.cells[i][j] == dg.STAIR_UP) {
						...
						instance.transform.scale(0.8f, 0.8f, 0.8f);
						instance.materials.get(0).set(
								ColorAttribute.createDiffuse(1.0f, 0.4f, 0.4f,
										0.9f));
						instances.add(instance);
					}

				} else {
					instance = new ModelInstance(topTile);
					instance.transform.translate(i * dg.cell_size * scale,
							0, -j * dg.cell_size * scale);
					instances.add(instance);
				}
	...
	public void act(float dt) {
		if (!loading) {
			...
			camera.position.set(player.x / 10 - 3, 14f, -player.y / 10 + 3 + 5);
			//camera.position.set(player.x / 10 - 3, 3f, -player.y / 10 + 3 + 5);
			camera.lookAt(player.x / 10 - 3, 0, -player.y / 10 + 3);
			camera.update();
			monkeyInstance.transform.setToTranslation(player.x / 10 - 3, 1,
					-player.y / 10 + 3);
