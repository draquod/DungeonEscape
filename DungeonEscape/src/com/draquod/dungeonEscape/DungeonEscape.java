...
public class DungeonEscape implements ApplicationListener {
	...
	public void create() {
		...
		if (Data.DEBUG)
			cam.zoom = 3.5f;
		cam.position.set(dg.cell_size * 10 * dg.n_cols / 2, dg.cell_size * 10
				* dg.n_rows / 2, 0);
		// cam.update();
	...
	public void render() {
		...
		if (!Data.MODE3D) {
			if (Data.DEBUG)
				cam.position.set(dg.cell_size * 10 * dg.n_cols / 2,
						dg.cell_size * 10 * dg.n_rows / 2, 0);
			if (!Data.DEBUG)
		...
		if (dg.cells[(int) player.x / dg.cell_size / 10][(int) player.y
				/ dg.cell_size / 10] == dg.STAIR_UP) {
			dg.CreateDungeon();
			ddd.dg = dg;
			if (Data.MODE3D) {
		...
		/*
		 * 
		 * if(dg.cells[(int) player.x/dg.cell_size/10][(int)
		 * player.y/dg.cell_size/10] == dg.STAIR_UP){ dg.CreateDungeon(); ddd.dg
		 * = dg; player.x = dg.begin.x*dg.cell_size*10 + dg.cell_size*10/2;
		 * player.y = dg.begin.y*dg.cell_size*10 + dg.cell_size*10/2;
		 * 
		 * } if(Data.DEBUG)
		 * cam.position.set(dg.cell_size*10*dg.n_cols/2,dg.cell_size
		 * *10*dg.n_rows/2,0); if(!Data.DEBUG)
		 * cam.position.set(player.x,player.y,0);
