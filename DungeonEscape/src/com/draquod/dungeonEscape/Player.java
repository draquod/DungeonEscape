...
public class Player extends Entity {
	...
	public void act(float dt) {
		...
		if (dg.cells[(int) newX / dg.cell_size / 10][(int) y / dg.cell_size
				/ 10] == 0
				|| dg.cells[(int) newX / dg.cell_size / 10][(int) y
						/ dg.cell_size / 10] == dg.PERIMETER) {

		} else {
			x = newX;
		}

		if (dg.cells[(int) x / dg.cell_size / 10][(int) newY / dg.cell_size
				/ 10] == 0
				|| dg.cells[(int) x / dg.cell_size / 10][(int) newY
						/ dg.cell_size / 10] == dg.PERIMETER) {
